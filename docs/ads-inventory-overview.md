...
任务先写入不可变 `data_version`，核对分仓记录数、库存数量、可用库存、库存金额、批次记录数、临期批次数、库存健康各异常类型的数量、有效批次明细，以及商品周转明细的记录数、库存、可用库存和近 30/90 天销量。全部一致后，才将 `ads_publish_batch` 中 `inventory_overview` 数据集的批次标记为 `ready`。
