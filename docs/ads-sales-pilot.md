...
- `ADS_BUILD_DATABASE_URL`：独立构建任务使用，仅授予五张销售 ADS 表所需的 `SELECT`、`INSERT`、`UPDATE` 权限。
...
初始化完成后，可以撤销构建账号的 DDL 权限。当前版本化构建只需要五张 ADS 表上的 `SELECT`、`INSERT`，以及发布批次表上的 `UPDATE`。
